...
public class AdminCourseController {
    ...
    // ─── VOIR les cours archivés ──────────────────────────────────────────
    // GET /api/admin/courses/archived
    @GetMapping("/archived")
    public ResponseEntity<List<CourseResponseDto>> getArchivedCourses() {
        return ResponseEntity.ok(courseService.getArchivedCoursesForAdmin());
    }
    ...
    public ResponseEntity<?> archiveCourse(@PathVariable Long courseId) {
        try {
            ...
            System.err.println("❌ [ARCHIVE ERROR] Course ID: " + courseId);
    ...
    // ─── DÉSARCHIVER un cours (admin) ─────────────────────────────────────
    // PUT /api/admin/courses/{courseId}/unarchive
    @PutMapping("/{courseId}/unarchive")
    public ResponseEntity<?> unarchiveCourse(@PathVariable Long courseId) {
        try {
            return ResponseEntity.ok(courseService.adminUnarchiveCourse(courseId));
        } catch (Exception e) {
            String msg = e.getMessage();
            return ResponseEntity.badRequest().body(java.util.Map.of("error", msg != null ? msg : "Erreur lors du désarchivage"));
        }
    }
